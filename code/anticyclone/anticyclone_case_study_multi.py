...
# ==================== 输出路径 ====================
OUTPUT_DIR = r'E:\GEO\pyproject\casestudy\test'
...
print("正在随机挑选 100 个点...")
while len(selected_points) < 200:
    traj_id = float(np.random.choice(poi_list['trajectory_id'].unique()))
    ...
    key = (traj_id, float(row['date']))
    if key not in seen_keys:
        seen_keys.add(key)
        selected_points.append({
            'traj_id': traj_id, 'idx_in_traj': idx, 'date': float(row['date']),
            'pressure_lon': float(row['pressure_lon']), 'pressure_lat': float(row['pressure_lat'])
        })

selected_df = pd.DataFrame(selected_points)
selected_df['traj_id'] = selected_df['traj_id'].astype(float)
selected_df['idx_in_traj'] = selected_df['idx_in_traj'].astype(int)
selected_df['date'] = selected_df['date'].astype(float)
print(f"成功挑选 {len(selected_df)} 个点")
...
def run_full_plot_for_row(traj_id, idx_in_traj, date, center_lon, center_lat, output_png):
    ...
    t_level = data_slice['t'].sel(pressure_level=level)
    ...
    csz = ax1.contour(lon2d, lat2d, t_vals,levels=t_levels, colors='black', linewidths=0.7, transform=ccrs.PlateCarree())
    ax1.clabel(cs1, inline=True, fontsize=7, fmt='%d')
    ax1.clabel(csz, inline=True, fontsize=7, fmt='%d')
...
'''
# ==================== 【手动单独画某一个】 ====================
# 改这里的数字 0~9（因为你现在只挑了10个点）
idx_to_plot = 55     # ←←←← 这里改成你想看的序号（从0开始）

row = selected_df.iloc[idx_to_plot]
traj_id     = float(row['traj_id'])
idx_in_traj = int(row['idx_in_traj'])
date        = float(row['date'])
center_lon  = float(row['pressure_lon'])
center_lat  = float(row['pressure_lat'])

time_str = str(int(date)).zfill(10)
year, month, day, hour = int(time_str[:4]), int(time_str[4:6]), int(time_str[6:8]), int(time_str[8:10])

output_png = os.path.join(OUTPUT_DIR, 
    f'circulation_thermo_500_1000_ID{traj_id}_{year}{month:02d}{day:02d}{hour:02d}_random_TEST.png')

print(f"\n=== 正在单独绘制第 {idx_to_plot} 个点（从0计） ===")
print(row)

run_full_plot_for_row(traj_id, idx_in_traj, date, center_lon, center_lat, output_png)
print("单独绘图完成！")
'''
# ==================== 主循环 ====================
for i, row in selected_df.iterrows():
    ...
    time_str = str(int(date)).zfill(10)
    year, month, day, hour = int(time_str[:4]), int(time_str[4:6]), int(time_str[6:8]), int(time_str[8:10])
    output_png = os.path.join(OUTPUT_DIR, f'circulation_thermo_500_1000_ID{traj_id}_{year}{month:02d}{day:02d}{hour:02d}_random.png')

    try:
        run_full_plot_for_row(traj_id, idx_in_traj, date, center_lon, center_lat, output_png)
        records.append({**row, 'year': year, 'month': month, 'day': day, 'hour': hour, 'png_file': os.path.basename(output_png)})
    except Exception as e:
        print(f"[ERROR] 轨迹 {traj_id} 时间 {date} 失败: {e}")
        failed.append({**row, 'error': str(e)})
    #run_full_plot_for_row(traj_id, idx_in_traj, date, center_lon, center_lat, output_png)
    #records.append({**row, 'year': year, 'month': month, 'day': day, 'hour': hour, 'png_file': os.path.basename(output_png)})
# ==================== 保存记录 ====================
pd.DataFrame(records).to_csv(os.path.join(OUTPUT_DIR, 'selected_points.csv'), index=False, encoding='utf-8-sig')
if failed:
    pd.DataFrame(failed).to_csv(os.path.join(OUTPUT_DIR, 'failed_points.csv'), index=False, encoding='utf-8-sig')
