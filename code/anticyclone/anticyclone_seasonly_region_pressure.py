...
# 统计每个季节、每个地区每年的平均中心气压
season_region_pressure = df.groupby(['Adjusted_Year', 'Season', 'region_name'])['center_pressure'].mean().reset_index()
season_region_pressure.columns = ['Year', 'Season', 'Region', 'Average Pressure']

# 计算每个季节、每个地区的多年平均中心气压
average_pressure = season_region_pressure.groupby(['Season', 'Region'])['Average Pressure'].mean().reset_index()

# 创建季节-地区矩阵
pivot_table = average_pressure.pivot(index='Season', columns='Region', values='Average Pressure').fillna(0)
