...
# 选取相关列：调整后的年份、季节、中心气压
df_filtered = df[['Adjusted_Year', 'Season', 'center_pressure']]

# 进行分组统计：按季节和调整后的年份计算平均气压
result = df_filtered.groupby(['Season', 'Adjusted_Year'])['center_pressure'].mean().reset_index()

# 重命名列名
result.columns = ["Season", "Year", "Average Pressure"]
...
for season in ['Spring', 'Summer', 'Fall', 'Winter']:
    ...
    pressures = season_data["Average Pressure"].values
    mean = pressures.mean()
    ...
    plt.plot(years, pressures, label=f"{chinese_season} (p={p:.3f})", color=color_map[season], linewidth=linewidth)

# 设置图表标题和标签（使用中文）
plt.title("北半球温带反气旋季节平均中心气压随时间变化", fontsize=12)
plt.xlabel("时间（年）", fontsize=9)
plt.ylabel("平均气压 (hPa)", fontsize=9)
...
plt.tight_layout()
plt.savefig('E:/GEO/result/anticyclone/pressure_seasonly.eps')
plt.show()