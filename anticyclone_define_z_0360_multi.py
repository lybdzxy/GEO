    ...
    for contour_data in leaf_contours:
        ...
        if not np.isnan(grid_h) and grid_h > 1200:  # 高中心阈值1200 gpm
            ...
            if center_tuple not in seen_centers:
                original_lon = grid_lon - 180 if grid_lon >= 180 else grid_lon + 180
...
def process_single_time(target_time, file_path_template='F:/ERA5/hourly/lvl/{hour}z/ERA5_{hour}z_lvl_{year}{month}.nc'):
    ...
    try:
        ...
        # 经度调整：将180-360映射到0-180，0-180映射到180-360
        ...
        lon_mask = (lon_adjusted >= 90) & (lon_adjusted <= 270)  # 限制在90°-270°范围内
        lon_filtered = lon_adjusted[lon_mask]
        height_filtered = height[:, lon_mask]

        # 将位势高度低于1000 gpm的部分设为nan（聚焦高值区域）
        height_above_1000 = np.where(height_filtered >= 1000, height_filtered, np.nan)

        # 2. 绘制等高线
        interval = 50  # 位势高度间隔50 gpm
        img_extent = [90, 270, -90, 90]  # 调整范围以匹配经度
        fig = plt.figure(figsize=(10, 10))
        ax = fig.add_subplot(111, projection=ccrs.PlateCarree())
        ax.set_extent(img_extent, crs=ccrs.PlateCarree())
        contours = plt.contour(
            lon_filtered, lat, height_above_1000,
        ...
        potential_high_centers = calculate_potential_high_centers(leaf_contours, lon_filtered, lat, height_above_1000)
...
if __name__ == "__main__":
    ...
    # 输出文件路径
    output_file = 'potential_high_centers_850hpa_z_0360.csv'
    ...
    # 日志文件路径
    error_log_path = "error_log_z_0360.txt"
    empty_log_path = "empty_results_log_z_0360.txt"
