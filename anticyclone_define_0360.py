...
# 确保 lat 和 pressure 数据的一致性（例如：如果有不同的纬度范围，确保索引匹配）
def calculate_high_centers(inner_contours, lon_filtered, lat_filtered, pressure_filtered):
    high_centers = {}
    for contour_data in inner_contours:
        ...
        # 保证中心点的经纬度在筛选范围内
        center_lon = np.clip(center_lon, lon_filtered.min(), lon_filtered.max())
        center_lat = np.clip(center_lat, lat_filtered.min(), lat_filtered.max())

        # 找到最接近中心的经纬度索引
        lon_idx = np.argmin(np.abs(lon_filtered - center_lon))
        lat_idx = np.argmin(np.abs(lat_filtered - center_lat))

        # 获取气压值
        grid_lon = lon_filtered[lon_idx]
        grid_lat = lat_filtered[lat_idx]
        grid_p = pressure_filtered[lat_idx, lon_idx]
...
# 调用时，确保传入筛选后的 lon_filtered 和 pressure_filtered
...
csv_path = 'high_pressure_systems_20200527_test4.csv'
