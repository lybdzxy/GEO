...
def process_date(date, pressure_groups, stream_groups):
    ...
    pres_values = pres_date['stream'].values
...
def main():
    # 文件路径
    pressure_file = 'potential_high_pressure_centers_slp_tot.csv'
    stream_file = 'potential_high_pressure_centers_stream_1000hpa_tot.csv'

    # 读取 CSV 文件，仅加载必要列
    pressure_df = pd.read_csv(pressure_file, usecols=['date', 'center_lon', 'center_lat', 'stream'])
    ...
    # 保存结果到 CSV
    result_df.to_csv('confirmed_high_pressure_centers_slp1000s.csv', index=False)
